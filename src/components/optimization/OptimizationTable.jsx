  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold text-pink-800">Optimization Results</h2>
        <div className="relative">
          <input
            type="text"
            placeholder="Filter tactics..."
            className="bg-pink-100 text-pink-700 placeholder-pink-400 rounded-lg pl-10 pr-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
            value={searchTerm}
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-pink-200 text-sm">
          ...
          <tbody className="divide-y divide-pink-100 text-pink-800">
            {filteredTactics.map((tactic) => {
              ...
              return (
                <motion.tr key={tactic} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">${newSpend.toFixed(2)}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
